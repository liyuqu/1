...
public class ExperimentRunner {
    ...
    private static int shaped=1;
    public static void main(String[] args) throws Exception {
        if (args.length < 1) {
            //System.err.println("Usage: java MyScript <pipeline>");
           // System.exit(1);
        }else {
        ...
        // Load common configs
            InputStream commonExperimentConfig = new FileInputStream("./src/main/java/expconfigs/common.properties");
    ...
    private static void runOpenLoopExperiment(int experimentTimeInSeconds, int warmupRequestsNum,
        ...
        InputStream specificExperimentConfig = new FileInputStream("./src/main/java/expconfigs/input-rate-exp-config.properties");
        ...
        for (int inputRate : inputRateValues) {
            ...
            System.out.println("************************************************");
            String outputFile = generateOutputFileName("input-rate", inputRate, modelReplicas, batchSize,shaped);
            FeedForwardPipeline
    ...
    private static void runCloseLoopExperiment(int experimentTimeInSeconds, int warmupRequestsNum,
                                               int maxInputRatePerThread) throws Exception {
        InputStream specificExperimentConfig = new FileInputStream("./src/main/java/expconfigs/batch-size-exp-config.properties");
        Properties props = new Properties();
        props.load(specificExperimentConfig);
        int modelReplicas = Integer.parseInt(props.getProperty("model_replicas"));
        ...
        for (int batchSize : batchSizeValues) {
            ...
            System.out.println("************************************************");
            String outputFile = generateOutputFileName("batch-size", inputRate, modelReplicas, batchSize,shaped);
            FeedForwardPipeline
    ...
    private static void runScalabilityExperiment(int experimentTimeInSeconds, int warmupRequestsNum,
            ...
            Exception {
        InputStream specificExperimentConfig = new FileInputStream("./src/main/java/expconfigs/scalability-exp-config.properties");
        Properties props = new Properties();
        props.load(specificExperimentConfig);
        int batchSize = Integer.parseInt(props.getProperty("batch_size"));
        int inputRate = Integer.parseInt(props.getProperty("input_rate"));
        System.out.println("MODEL REPLICAS = " + modelReplicas);
        String outputFile = generateOutputFileName("scalability", inputRate, modelReplicas, batchSize,shaped);
        FeedForwardPipeline
                .run(inputRate, batchSize, experimentTimeInSeconds, warmupRequestsNum, true, maxInputRatePerThread,
                        outputFile,shaped);
        Thread.sleep(10000);
